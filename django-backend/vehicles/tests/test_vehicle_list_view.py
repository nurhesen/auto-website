...
class VehiclesListViewTest(APITestCase):
    def create_vehicle(self, title='Test Vehicle', color='Black', condition=1, fuel=1, price=10000,
    ...
    def setUp(self):
        ...
        self.vehicle2 = self.create_vehicle(condition=2, price=18000, currency=2, brand_model=vehicle_model)
        self.vehicle3 = self.create_vehicle(condition=1, price=18000, currency=1, brand_model=vehicle_model)


    def test_filter_vehicles_max_min(self):
        url = reverse('vehicles-list')
        data = {'min_year': 2018, 'max_year': 2020, 'min_price': 15000, 'max_price': 20000}
        ...
        self.assertEqual(len(response.data), 2)  # Expecting 2 vehicles to match the filter criteria
    ...
    def test_filter_vehicles_exact(self):
        ...
        # Check if the filtered data is correct
        self.assertEqual(len(response.data), 1)  # Expecting 1 vehicle to match the filter criteria

    def test_filter_vehicles_empty(self):
        # Giving empty query list and expecting to return all vehicles
        ...
        # Check if the filtered data is correct
        self.assertEqual(len(response.data), 3)  # Expecting 3 vehicle to match the filter criteria
